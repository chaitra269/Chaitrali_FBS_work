...
class Product:
    ...
    def __del__(self):
        print(f"Product '{self.pname}'removed from memory.")

    def ShowBook(self):
        print("\n**********Product Details***********")
        print(f"Product ID: {self.pid}\tProduct Name: {self.pname}\tOriginal Price: {self.price:.2f}\tDiscounted Price: {self.get_discounted_price():.2f}Quantity: {self.quantity}")
